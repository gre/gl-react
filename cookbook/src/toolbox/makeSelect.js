...
export default (choices: Array<{ key: string, label: string }>) => class Radios extends Component {
  props: {
    value: string,
    onChange: (c:string)=>any,
  };
  onChange = (e: any) => this.props.onChange(e.target.value);
  random = () => this.props.onChange(choices[Math.floor(Math.random()*choices.length)].key);
  prev = () => {
  ...
  render() {
    ...
    return (
      <div style={styles.root}>
        <button style={styles.random} onClick={this.prev}>←</button>
        <select style={styles.select} value={value} onChange={this.onChange}>
        {choices.map(({ key, label }) =>
          <option key={key} value={key}>{label}</option>)}
        </select>
        <button style={styles.random} onClick={this.next}>→</button>
      </div>
    );
  }
};
