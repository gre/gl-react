...
function DesertPassage({ width, height }: { width: number; height: number }) {
...
function ShowCaptured({ t }: { t: any }) {
  return (
    <Surface width={200} height={200}>
      <Node shader={crtShaders.copy} uniforms={{ t }} />
    </Surface>
  );
}
...
export default function DemoDesertCRT({
  ...
  const desertRef = useRef<any>(null);
  const [surfacePixels, setSurfacePixels] = useState<any>(null);
  const [desertPixels, setDesertPixels] = useState<any>(null);

  const onCapture = useCallback(() => {
    if (surfaceRef.current) {
      setSurfacePixels(surfaceRef.current.capture());
    }
    if (desertRef.current) {
      setDesertPixels(desertRef.current.capture());
    }
  }, []);
  ...
  return (
    <div>
      ...
      >
        <Bus ref={desertRef}>
          <DesertPassage width={128} height={128} />
        </Bus>
