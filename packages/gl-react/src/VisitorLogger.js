...
const aggregateInfo = (info) =>
  ...
  : [ String(
        info.dependency && info.dependency.getGLName()
        || info.initialObj
...
export default class VisitorLogger extends Visitor {
  ...
  onNodeDrawSkipped(node: Node) {
    log(node.getGLName()+" redraw _skipped_: "+(
  ...
  onNodeSyncDeps(node: Node, additions: Array<Bus | Node>, deletions: Array<Bus | Node>) {
  ...
  onNodeDraw(node: Node, preparedUniforms: Array<*>) {
    ...
    log(
      "_size_ `"+w+"`x`"+h+"` "+
      "_clear_ `"+JSON.stringify(clear)+"` "+
      "_blendFunc_ `"+JSON.stringify(blendFunc)+"`");
    ...
    preparedUniforms.forEach(obj => {
      ...
      log(`${spaces}*${type==="UNKNOWN" ? "[c='color:red']UNKNOWN[c]" : type}* _${key}_ = ${values}`,
